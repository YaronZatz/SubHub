...
function MobileTabBar({ pathname, isLoggedIn, onAuthRequired }: TabBarProps) {
  const router = useRouter();
  const [showComingSoon, setShowComingSoon] = useState(false);

  const tab = (href: string, label: string, requiresAuth: boolean, icon: React.ReactNode) => {
    const isActive = pathname === href || pathname.startsWith(href + '/');
    const cls = `flex flex-col items-center gap-0.5 flex-1 py-2 text-[10px] font-bold transition-colors ${isActive ? 'text-[#4A7CC7]' : 'text-slate-500'}`;
    const handleClick = () => {
  ...
  const handlePost = () => { setShowComingSoon(true); setTimeout(() => setShowComingSoon(false), 2500); };

  return (
    <>
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-slate-200 flex md:hidden">
        ...
        <button onClick={handlePost} className="flex flex-col items-center gap-0.5 flex-1 py-2 text-[10px] font-bold text-slate-500">
          <div className="w-8 h-8 -mt-5 rounded-full bg-[#4A7CC7] flex items-center justify-center shadow-md">
      ...
      {showComingSoon && (
        <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-[60] bg-slate-900 text-white text-xs font-bold px-4 py-2 rounded-full shadow-lg pointer-events-none md:hidden">
          Posting coming soon
        </div>
      )}
    </>
